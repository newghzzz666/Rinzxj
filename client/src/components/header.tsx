...
// 定义通用按钮样式
const ICON_BTN_CLASS = "w-9 h-9 rounded-full flex items-center justify-center text-neutral-500 hover:text-black dark:hover:text-white hover:bg-neutral-100 dark:hover:bg-white/10 transition-all duration-200 active:scale-95";
...
export function Header({ children }: { children?: React.ReactNode }) {
    ...
    return useMemo(() => (
        <>
            {/* Header 容器：灵动岛风格
                1. 桌面端 (md): 距离顶部 24px (top-6), 自适应宽度, 圆角胶囊
                2. 移动端: 贴顶 (top-0), 全宽
            */}
            <div className="fixed z-50 left-0 right-0 flex justify-center transition-all duration-500 top-0 w-full md:top-6 md:w-auto">
                
                <div className="relative group w-full md:w-auto">
                    {/* 背景光晕层 (仅桌面端显示) */}
                    <div className="hidden md:block absolute -inset-0.5 bg-gradient-to-r from-orange-500/50 to-purple-600/50 rounded-full opacity-20 group-hover:opacity-50 blur transition duration-700"></div>
                    
                    {/* 核心内容层 */}
                    <div className="relative flex items-center justify-between px-4 py-2 bg-white/90 dark:bg-[#09090b]/90 backdrop-blur-xl border-b md:border border-neutral-200/50 dark:border-white/10 shadow-lg md:shadow-2xl md:rounded-full min-w-full md:min-w-[700px] transition-all duration-300">
                        
                        {/* 1. 左侧 Logo */}
                        <Link aria-label={t('home')} href="/" className="flex items-center gap-3 mr-4 group hover:opacity-80 transition-opacity">
                            <img src={process.env.AVATAR} alt="Avatar" className="w-9 h-9 rounded-full border border-neutral-200 dark:border-neutral-700 group-hover:border-orange-500 transition-colors duration-300" />
                            <div className="flex flex-col">
                                <span className="font-bold text-sm tracking-wide dark:text-white group-hover:text-orange-500 transition-colors">
                                    {process.env.NAME}
                                </span>
                            </div>
                        </Link>
        ...
        <>
            {when &&
                <Link href={href}
                    ...
                    text-sm font-bold transition-all duration-200 px-4 py-1.5 rounded-full relative group
                    ${selected 
                        ? "text-orange-500" 
                ...
                >
                    ...
                    {/* 桌面端的底部光标动画 */}
                    {!menu && (
                        <span className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-0 h-0.5 bg-orange-500 rounded-full transition-all duration-300 group-hover:w-1/3 ${selected ? 'w-1/3' : ''}`}></span>
                    )}
                </Link>}
        </>
    )
...
function Menu() {
    ...
    return (
        <div className="flex items-center">
            ...
            >
                <div className="bg-white dark:bg-[#09090b] w-[80vw] max-w-[300px] p-4 m-4 rounded-2xl shadow-2xl border border-neutral-200 dark:border-neutral-800 flex flex-col gap-2 ring-1 ring-black/5">
                    {/* 移动端菜单内部头部 */}
                    <div className="flex justify-between items-center pb-4 border-b border-neutral-100 dark:border-white/10 mb-2">
                        <span className="text-xs font-bold text-neutral-400 uppercase tracking-widest">Menu</span>
...
function LanguageSwitch({ className }: { className?: string }) {
    ...
    return (
        <div className={className}>
            ...
            >
                <div className="bg-white dark:bg-[#09090b] border border-neutral-100 dark:border-white/10 rounded-xl shadow-xl overflow-hidden min-w-[80px] mt-2 p-1">
...
function SearchButton({ className, onClose }: { className?: string, onClose?: () => void }) {
    ...
    return (<div className={className}>
        ...
        >
            <div className="bg-white dark:bg-[#09090b] w-[90vw] md:w-[600px] p-4 rounded-2xl shadow-2xl border border-neutral-200 dark:border-neutral-800 flex gap-3 items-center animate-in fade-in zoom-in-95 duration-200 ring-1 ring-white/10">
        ...
        <div className={className + " flex items-center"}>
            {profile?.avatar ? (
                <div className="w-9 h-9 relative group cursor-pointer ml-2">
                    <img src={profile.avatar} alt="Avatar" className="w-9 h-9 rounded-full border border-neutral-200 dark:border-neutral-700 transition-transform group-hover:scale-105" />
