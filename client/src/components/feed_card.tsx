...
export function FeedCard({ id, title, avatar, draft, listed, top, summary, hashtags, createdAt, updatedAt }:
    ...
    return useMemo(() => (
        <>
            <Link href={`/feed/${id}`} target="_blank" className="w-full rounded-2xl glass-card my-2 p-6 duration-300 block">
                {avatar &&
                ...
                <p className="space-x-2">
                    ...
                    {top === 1 && <span className="text-theme text-sm font-medium">
                        {t('article.top.title')}
                    </span>}
                </p>
                <p className="text-pretty overflow-hidden dark:text-neutral-400 mt-2">
                    {summary}
                </p>
                {hashtags.length > 0 &&
                    <div className="mt-3 flex flex-row flex-wrap justify-start gap-x-2 gap-y-1">
